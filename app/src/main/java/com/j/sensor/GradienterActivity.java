...
public class GradienterActivity extends Activity implements SensorEventListener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        // 获取水平仪的主组件
        show = (GradienterView) findViewById(R.id.gradienterView);
        // 获取传感器管理服务
    ...
    public void onSensorChanged(SensorEvent event) {
        ...
        switch (sensorType) {
            case Sensor.TYPE_ORIENTATION:
                ...
                int x = (show.background.getWidth() - show.bubble.getWidth()) / 2;
                int y = (show.background.getHeight() - show.bubble.getHeight()) / 2;
                // 如果与Z轴的倾斜角还在最大角度之内
                if (Math.abs(zAngle) <= MAX_ANGLE) {
                    // 根据与Z轴的倾斜角度计算X坐标的变化值
                    // （倾斜角度越大，X坐标变化越大）
                    int deltaX = (int) ((show.background.getWidth() - show.bubble
                ...
                // 如果与Z轴的倾斜角已经小于负的MAX_ANGLE，气泡应到最右边
                    ...
                    x = show.background.getWidth() - show.bubble.getWidth();
                }
                // 如果与Y轴的倾斜角还在最大角度之内
                if (Math.abs(yAngle) <= MAX_ANGLE) {
                    // 根据与Y轴的倾斜角度计算Y坐标的变化值
                    // （倾斜角度越大，Y坐标变化越大）
                    int deltaY = (int) ((show.background.getHeight() - show.bubble
                            .getHeight()) / 2 * yAngle / MAX_ANGLE);
                    y += deltaY;
                }
                // 如果与Y轴的倾斜角已经大于MAX_ANGLE，气泡应到最下边
                else if (yAngle > MAX_ANGLE) {
                    y = show.background.getHeight() - show.bubble.getHeight();
                }
    ...
    private boolean isContain(int x, int y) {
        ...
        // 计算水平仪仪表盘的圆心坐标X、Y
        int backCx = show.background.getWidth() / 2;
        int backCy = show.background.getWidth() / 2;
        ...
        return distance < (show.background.getWidth() - show.bubble.getWidth()) / 2;
    }
}
